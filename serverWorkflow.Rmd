...
**Questions**: 
is it necessary to save all three of these, or can we just save `tempDataSync`? 
**YES**

How will validation data be checked?
**Additional script later will calculate the predictions for those sites excluded from the analysis (validation data - `tempDataSyncValid` & `tempDataSyncValidS`) then check how well the observed match the predictions.**
...
The structure of the `evalRows` file (loaded with `tempDataSync.RData` is:

```{r}
str(evalRows)
```

The structure of the `firstObsRows` file (loaded with `tempDataSync.RData` is:
...
```{r}
load('../../dataOut/covariate-list.RData')
str(cov.list)
```
...
```
$ Rscript summarize ./tempDataSync.RData ./jags.RData ./coef.RData
```
```{r coef structure}
# load from existing modSummary in dataOut/
load('../../dataOut/coef.RData')
str(coef.list)
```
...
Rscript summarize $1/tempDataSync.RData $1/jags.RData $1/coef.RData
```
